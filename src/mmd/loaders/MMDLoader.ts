...
class MaterialBuilder {
		...
		for ( let i = 0; i < data.metadata.materialCount; i ++ ) {
			...
			if ( data.metadata.format === 'pmd' ) {
				...
				if ( material.toonIndex === - 1 || material.toonFlag !== 0 ) {
					...
					// PMX files may list an MMD standard toon texture in the
					// model's texture table (for example, tex/toon10.bmp).
					// Those files are normally supplied by MMD itself, so use
					// the loader's built-in replacement instead of requesting a
					// missing external asset.
					isDefaultToon = this._isDefaultToonTexture( toonFileName );
	...
	_isDefaultToonTexture( name ) {
		...
		return /(?:^|[\\/])toon(10|0[0-9])\.bmp$/i.test( name );
